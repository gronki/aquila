program test_legacy_align
  ...
  write_stack: block
    ...
    call ftphps(66, -storage_size(real(1, fp)), 2, shape(imsta), errno)
    call ftppre(66, 1, 1, size(imsta), real(imsta / size(imali,3), fp), errno)
  ...
  write_aligned: block
    ...
    call ftphps(67, -storage_size(real(1, fp)), 3, shape(imali), errno)
