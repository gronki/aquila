module transforms
  ...
  type, extends(transform) :: translation
    real(fp) :: x = 0, y = 0
  contains
    procedure :: apply => transl_apply
    procedure :: apply_inv => transl_apply_inv
    procedure :: det => transl_det
    procedure :: components => transl_get_components
    procedure :: pder => transl_get_pder
  end type

  !----------------------------------------------------------------------------!

  type, extends(transform) :: affine_transform
    real(fp) :: x = 0, xx = 1, xy = 0
    real(fp) :: y = 0, yx = 0, yy = 1
  contains
...
contains
  ...
  elemental subroutine transl_apply(t, x1, y1, x2, y2)
    class(translation), intent(in) :: t
    real(fp), intent(in)  :: x1, y1
    real(fp), intent(out) :: x2, y2
    x2 = x1 + t % x
    y2 = y1 + t % y
  ...
  elemental subroutine transl_apply_inv(t, x1, y1, x2, y2)
    class(translation), intent(in) :: t
    real(fp), intent(in)  :: x1, y1
    real(fp), intent(out) :: x2, y2
    x2 = x1 - t % x
  ...
  elemental function transl_det(t) result(det)
    class(translation), intent(in) :: t
    real(fp) :: det
    det = 1
  end function

  subroutine transl_get_components(t,c)
    class(translation), intent(in) :: t
    real(fp), intent(out) :: c(:)
    C(1) = T % X
    C(2) = T % Y
  ...
  subroutine transl_get_pder(t,x,y,dx,dy)
    class(translation), intent(in) :: t
    real(fp), intent(in) :: x,y
    real(fp), dimension(:), intent(out) :: dx,dy
    dx(1) = 1
...
contains
  ...
  pure subroutine project(t, dest, src)
    class(transform), intent(in) :: t
    real(fp), dimension(:,:), intent(in)  :: src
    real(fp), dimension(:,:), intent(out) :: dest
    integer   :: id, jd
    integer   :: ik, jk
    real(fp)  :: is, js
    ...
    do jd = 1, size(dest,2)
      do id = 1, size(dest,1)
        call t % apply(real(id,fp), real(jd,fp), is, js)
