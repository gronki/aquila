...
subroutine read_image_data(self, un, ftiostat)
   class(frame_t), target, intent(inout) :: self
   ...
   flatptr(1:size(self % data)) => self % data
...
subroutine write_fits_quick(fn, buf, errno)
   ...
   integer, intent(inout), optional :: errno
   integer :: ftiostat, un, iostat, bufshape(2)
   real(buf_k), contiguous, target :: buf(:,:)
